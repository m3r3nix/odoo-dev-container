[![Build and Push Odoo Dev Container](https://github.com/m3r3nix/odoo-dev-container/actions/workflows/docker-publish.yml/badge.svg?branch=v19)](https://github.com/m3r3nix/odoo-dev-container/actions/workflows/docker-publish.yml)
...
- Images are tagged not only with `latest` but also with the version number of the downloaded Debian Odoo installer package. This approach allows you to spin up a Codespace with a previous build version in case the latest Odoo version has a serious bug. Simply select an image, for example, `odoo-v19-community`, and look for available tags like `19.0.20260103` in your Container Registry. Then, adjust the tag in `docker-compose.yml` accordingly.
...
1. Update the current URL in `.devcontainer/URL.conf` to the latest Community version:  
    <https://nightly.odoo.com/19.0/nightly/deb/odoo_19.0.latest_all.deb>  
...
2. If required, update the image name/tag in the `docker-compose.yml` file accordingly, for example `odoo-v19-community:latest`.
...
3. If required, update the image name/tag in the `docker-compose.yml` file accordingly, for example `odoo-v19-enterprise:latest`.
...
1. The default branch should always contain the latest version. Therefore, create a new branch named after the latest version (e.g., `v20`) before making any changes.
